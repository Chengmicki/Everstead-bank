...
confirmBuyBtn.addEventListener('click', () => {
  ...
  if (currentBalance >= totalCost) {
    ...
    alert(`Purchase successful! You bought BTC worth $${usdAmount.toFixed(2)} (Fee: $${fee.toFixed(2)})`);
    resetBuyForm();
...
confirmSellBtn.addEventListener('click', () => {
  ...
  if (btcAmount <= currentBtcBalance) {
    ...
    alert(`Sale successful! You sold ${btcAmount.toFixed(8)} BTC for $${(usdAmount - fee).toFixed(2)} (Fee: $${fee.toFixed(2)})`);
    resetSellForm();
